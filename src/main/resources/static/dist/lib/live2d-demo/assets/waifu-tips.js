...
// 后端接口
live2d_settings['modelAPI']             = '//live2d.fghrsh.net/api/';   // 自建 API 修改这里 可选 '//live2d.fghrsh.net/api/'
live2d_settings['tipsMessage']          = 'waifu-tips.json';            // 同目录下可省略路径
live2d_settings['hitokotoAPI']          = 'hitokoto.cn';                  // 一言 API，可选 'lwl12.com', 'hitokoto.cn', 'jinrishici.com'(古诗词)

// 默认模型
live2d_settings['modelId']              = 2;            // 默认模型 ID，可在 F12 控制台找到  1~7 默认 1
live2d_settings['modelTexturesId']      = 42;           // 默认材质 ID，可在 F12 控制台找到 默认 53
...
//看板娘样式设置
live2d_settings['waifuSize']            = '360x320';    // 看板娘大小,例如 '280x250'(默认), '600x535'
live2d_settings['waifuTipsTimeOut']     = 5000;         // 提示框显示时间(毫秒),例如 '4000'(默认)   
live2d_settings['waifuTipsFadeOutTime'] = 400;          // (默认)提示框渐消失时间 (毫秒),例如 '200'(默认)
live2d_settings['waifuTipsSize']        = '250x130';     // 提示框大小，例如 '250x70'(默认), '570x150'
live2d_settings['waifuFontSize']        = '20px';       // 提示框字体，例如 '12px'(默认), '30px'
live2d_settings['waifuToolFont']        = '20px';       // 工具栏字体，例如 '14px'(默认), '36px'
live2d_settings['waifuToolLine']        = '36px';       // 工具栏行高，例如 '20px'(默认), '36px'
live2d_settings['waifuToolTop']         = '-80px'         // 工具栏顶部边距，例如 '0px'(默认), '-60px'
live2d_settings['waifuMinWidth']        = '768px';      // 面页小于 指定宽度 隐藏看板娘，例如 'disable'(禁用), '768px'(默认)
live2d_settings['waifuEdgeSide']        = 'left:0';     // 看板娘贴边方向，例如 'left:0'(靠左 0px)(默认), 'right:30'(靠右 30px)
live2d_settings['waifuDraggable']       = 'axis-x';    // 拖拽样式，例如 'disable'(禁用)(默认), 'axis-x'(只能水平拖拽), 'unlimited'(自由拖拽)
live2d_settings['waifuDraggableRevert'] = false;         // 松开鼠标还原拖拽位置，可选 true(真)(默认), false(假)
...
live2d_settings['aboutPageUrl']         = '/about';   // 关于页地址, '{URL 网址}' ,例如 'https://www.fghrsh.net/post/123.html'(来源-大佬博客)
...
function hideMessage(timeout) {
	if (timeout === undefined) timeout = live2d_settings.waifuTipsTimeOut; //默认提示框显示时间
    $('.waifu-tips').delay(timeout).fadeTo(live2d_settings.waifuTipsFadeOutTime, 0 , function() {
    	//$('.waifu-tips').stop().css('opacity',1);//opacity不透明级别1=>完全显示
    	sessionStorage.removeItem('waifu-text') ;
    });//隐藏
    
//    window.setTimeout(function() {
//    	$('.waifu-tips').fadeTo(200, 0);//隐藏
//	}, timeout);
}
...
function initModel(waifuPath, type) {
    ...
    } loadModel(modelId, modelTexturesId); //模型
}
...
function loadTipsMessage(result) {
    ...
    $.each(result.click, function (index, tips){
        $(document).on("click", tips.selector, function (){//鼠标点击事件
            var text = getRandText(tips.text);
            text = text.render({text: $(this).text()});
    ...
    if (live2d_settings.showF12OpenMsg) {
    	...
    	//var re = /x/;
    	//console.log(re);
    	//re.toString = function() {/*do you function*/};
    }
    ...
    $('.waifu-tool .fui-cross').click(function(){
        ...
        showMessage(getRandText(result.waifu.hidden_message), 3000, true);
