...
+ [glibc](https://www.gnu.org/software/libc/) —— c语言标准库 （协议：`LGPL`，动态链接不传染）
+ [libstdc++](https://gcc.gnu.org/onlinedocs/libstdc++/) —— c++语言标准库 （协议：`GPL-3.0-with-GCC-exception`，[使用未修改的GCC编译不传染](https://gnu.ac.cn/licenses/gcc-exception-3.1-faq.html)）
+ libgcc —— todo
...
### lambda函数 {id=id-xx-function-lambda}

lambda表达式用于快速定义一个匿名函数对象（也称“闭包，Closure”）。
...
+ 参数列表
+ 可选限定符 —— 可以缺省
+ 返回类型 —— 可以缺省，自动识别函数代码中的return类型
```

e.g.

```cpp
TEST(test_function, lambda_demo) {
  int x = 7;
  float y = 3.0;
  auto p = [x, y](int a, int b) -> float {
    return x * y + a * b;
  };
  ASSERT_EQ(p(10, 20), 221);
}
```
...
```cpp
TEST(test_function, lambda_struct) {
  int x = 7;
  float y = 3.0;
  struct {
    int m_x;
...
```cpp
TEST(test_function, lambda_demo_mutable) {
  int x = 7;
  float y = 3.0;
  auto p = [x, y](int a, int b) mutable -> float {
    x = 1; // 默认函数是const不可修改的
    return x * y + a * b;
  };
  ASSERT_EQ(p(10, 20), 203);
}
```
...
int main(void) {
  ...
  float y = 3.0;
  float z = -1;
  auto p = [=](int a, int b)->float { // 全部按值捕获
    return x*y + a*b;
  };
  auto p2 = [=, &y](int a, int b)->float { // 全部按值捕获，但也支持特殊指定
    y++;
    return x*y + a*b;
  };
}
...
auto createLambda() {
  float x = 1.0;
  float y = 2.0;
  return [&](float a) { // address of stack memory associated with local variable 'x' returned [-Wreturn-stack-address]GCC
    return a*x + y;
  };
};
TEST(test_function, disable_lambda_demo) {
  auto f = createLambda();
  std::cout<<f(2.0)<<std::endl;
}
```

在`lambda`中引用类`this`和`*this`值

```cpp
TEST(test_function, lambda_class_this) {
  class X {
  int m_v = 1;
  public:
    auto genLambda() {
      return [this](int a) {
        std::cout<<"ptr:"<<(unsigned long long)this<<std::endl;
        std::cout<<"value:"<<m_v<<std::endl;
        std::cout<<"a:"<<a<<std::endl;
      };
    }
    auto genLambda_star() {
      // *this = 复制新的类。注意，这种模式不可修改
      return [*this](int a) {
        std::cout<<"ptr:"<<(unsigned long long)this<<std::endl;
        std::cout<<"value:"<<m_v<<std::endl;
        std::cout<<"a:"<<a<<std::endl;
  ...
  X x;
  std::cout<<"ptr:"<<(unsigned long long)&x << std::endl;
  x.genLambda()(66);
  x.genLambda_star()(99);
}
```
...
[link_lambda函数](#id-xx-function-lambda)

```cpp
TEST(test_function, lambda_callback) {
  std::vector<float> numbers{1.1, 2.0, 3.5, -1.4};
  std::sort(numbers.begin(), numbers.end(), [](float a, float b) {return a<b;});
  std::cout<<"[";
  for(auto v:numbers)
    std::cout<<v<<",";
  std::cout<<"]"<<std::endl;
...
## C标准库（C Standard Library，glibc）

参考：

+ C 标准库头文件 <https://cppreference.cn/w/c/header>

Demo：

+ <RepoLink path="/code/demo-c-base/demo-06-glibc" />

## C++标准库（C++ Standard Library，libstdc++）

参考：

+ C++ 标准库头文件 <https://cppreference.cn/w/cpp/header>

Demo：

+ <RepoLink path="/code/demo-c-base/demo-06-libstdcpp" />
