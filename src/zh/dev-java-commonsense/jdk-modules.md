...
+ 北理金旭亮|52-亲身尝试Java模块化开发 - <https://www.bilibili.com/video/BV1eD4y1x7Sv/>

<!-- more -->

## 模块查看

在Java 9之前，Java标准库由一个单一巨大的rt.jar组成，里面按包路径划分了很多不同的功能模块。其关系如下图：

![rt.jar](https://bugs.openjdk.org/secure/attachment/72525/jdk.png)

::: tip

在Java 9之前，rt.jar内部文件列表可以通过如下命令查看：

```bash
jar --list --file rt.jar
```
...
└── src
    └── hello.world
        ...
        └── org
            └── example
                └── Main.java
$ (cd bin/hello.world; java org.example.Main) # 运行结果
xml
...
hello.world -> java.base
hello.world -> java.xml
$ java -jar hello.jar # 运行结果
xml
$ java -cp hello.jar org.example.Main # 运行结果（方式二）
xml
```

todo `jdeps --generate-module-info . hello.jar` <https://dev.java-lang.cn/learn/jvm/tools/core/jdeps/>
...
$ java -p hello.jar -m hello.world # 简写
xml
```
...
使用 `--add-exports` 和 `--add-opens` 绕过强封装 <https://dev.java-lang.cn/learn/modules/add-exports-opens/>

```bash
# 添加模块使用
# sun.util.BuddhistCalendar calendar = new sun.util.BuddhistCalendar();
javac
