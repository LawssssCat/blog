...
class XRequest {
  ...
  // T = Data
  request<T>(config: XRequestConfig<T>) {
  ...
  post<T>(config: XRequestConfig<T>) {
    return this.request({ ...config, method: 'POST' });
  }
  delete<T>(config: XRequestConfig<T>) {
    return this.request({ ...config, method: 'DELETE' });
  }
  patch<T>(config: XRequestConfig<T>) {
